...
const TopDoctors = () => {
  ...
  return (
    <div className='container mx-auto flex flex-col items-center gap-4 my-8 md:my-16 text-gray-900 px-4 md:px-6 lg:px-8' >
      <h1 className='text-2xl md:text-3xl lg:text-4xl font-medium text-center' >Our Best Doctors</h1>
      <p className='w-full md:w-2/3 lg:w-1/3 text-center text-sm md:text-base text-gray-600' >You check out the list of our trusted doctors.</p>
      <div className='w-full grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 md:gap-6 pt-5' >
        {doctors.slice(0,10).map((item,index)=>(
          <div 
            onClick={() => navigate(`/appointment/${item._id}`)} 
            className='bg-white border border-blue-200 rounded-xl overflow-hidden cursor-pointer hover:translate-y-[-10px] transition-all duration-500 hover:shadow-xl' 
            key={index}
          >
            <div className='relative w-full pt-[75%]'>
              <img 
                className='absolute top-0 left-0 w-full h-full object-cover bg-[#eaf9ff]' 
                src={item.image} 
                alt={item.name}
                loading="lazy" 
              />
